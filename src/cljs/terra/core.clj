...
;
;   Additions and Updates made by Paul deGrandis
...
(defmacro str [& xs]
  (let [strs (->> (repeat (count xs) "cljs.core.str(~{})")
                  (interpose ",")
                  (apply core/str))]
   (concat (list 'host* (core/str "table.concat({" strs "})")) xs)))
...
(defn simple-test-expr? [env ast]
  (core/and
    (#{:var :invoke :constant :dot :terra} (:op ast))
    ('#{boolean seq} (cljs.terra.analyzer/infer-tag env ast))))
  ...
  ([x & next]
    (let [forms (concat [x] next)]
      (if (every? #(simple-test-expr? &env %)
            (map #(cljs.terra.analyzer/analyze &env %) forms))
        (let [and-str (->> (repeat (count forms) "(~{})")
                        (interpose " and ")
                        (apply core/str))]
          (bool-expr `(~'host* ~and-str ~@forms)))
        `(let [and# ~x]
           (if and# (and ~@next) and#))))))
...
(defmacro coercive-not [x]
  (bool-expr (core/list 'host* "(not ~{})" x)))

;; internal - do not use.
(defmacro coercive-not= [x y]
  (bool-expr (core/list 'host* "(~{} ~= ~{})" x y)))
...
;; internal - do not use
(defmacro terra-arguments []
  (core/list 'host* "{ ... }"))

(defmacro terra-delete [obj key]
  (core/list 'host* "~{}[~{}] = nil" obj key))

(defmacro true? [x]
  (bool-expr (core/list 'host* "~{} == true" x)))

(defmacro false? [x]
  (bool-expr (core/list 'host* "~{} == false" x)))

;; TODO These are Lua Arrays - just Tables with standard indexes starting at 1
;;    We need to make `tarray` and `tvector`
(defmacro array? [x]
  (bool-expr (core/list 'host* "(~{}.constructor == cljs.core.Array)" x)))

(defmacro string? [x]
  (bool-expr (core/list 'host* "(type ~{} == 'string')" x)))

;; TODO: x must be a symbol, not an arbitrary expression
(defmacro exists? [x]
  (bool-expr
    (core/list 'host* "(~{} ~= nil)"
      (vary-meta x assoc :cljs.terra.analyzer/no-resolve true))))

(defmacro undefined? [x]
  (bool-expr (core/list 'host* "(nil == ~{})" x)))

(defmacro identical? [a b]
  (bool-expr (core/list 'host* "(~{} == ~{})" a b)))
...
(defmacro instance? [t o]
  ...
  (bool-expr (if (clojure.core/symbol? t)
               (core/list 'host* "(type ~{} == ~{})" o t)
               `(let [t# ~t o# ~o]
                  (~'host* "(type ~{} == ~{})" o# t#)))))

(defmacro number? [x]
  (bool-expr (core/list 'host* "(type ~{} == 'number')" x)))
...
;;TODO: Here - these should *ideally* be Terra arrays, but
;; in order for them to work with Lua Tables-as-arrays AND
;; Terra low-level arrays, we need a protocol.
;; If it was a protocol, we can make it work with Terra Vectors

(defmacro aget
  ([a i]
     (core/list 'host* "(~{}[~{}])" a (inc i)))
  ([a i & idxs]
     (let [lua-idxs (map inc (conj idex i))
           astr (apply core/str (repeat (count lua-idxs) "[~{}]"))]
      `(~'host* ~(core/str "(~{}" astr ")") ~a ~@lua-idxs))))

(defmacro aset
  ([a i v]
    (core/list 'host* "(~{}[~{}] = ~{})" a (inc i) v))
  ([a idx idx2 & idxv]
    (let [lua-idxs (map inc (butlast idxv idx2 idx))
          v (last idxv)
          astr (apply core/str (repeat (count lua-idxs) "[~{}]"))]
      `(~'host* ~(core/str "(~{}" astr " = ~{})") ~a ~lua-idxs ~v))))

(defmacro ^::ana/numeric +
  ([] 0)
  ([x] x)
  ([x y] (core/list 'host* "(~{} + ~{})" x y))
  ([x y & more] `(+ (+ ~x ~y) ~@more)))

;; In terra, casts can only happen at specific places.
;; For now, these will remain the same
;;  TODO: Come back here and figure out if casting is appropriate
;;     We would need to know scope
...
(defmacro ^::ana/numeric <
  ([x] true)
  ([x y] (bool-expr (core/list 'host* "(~{} < ~{})" x y)))
...
(defmacro ^::ana/numeric max
  ...
  ([x y] `(let [x# ~x, y# ~y]
            (~'host* "((~{} > ~{}) and ~{} or ~{})" x# y# x# y#)))
  ([x y & more] (core/list 'host* "(math.max(~{}))" (apply str (interpose ", " (conj more y x))))))

(defmacro ^::ana/numeric min
  ([x] x)
  ([x y] `(let [x# ~x, y# ~y]
            (~'host* "((~{} < ~{}) and ~{} or ~{})" x# y# x# y#)))
  ([x y & more] (core/list 'host* "(math.min(~{}))" (apply str (interpose ", " (conj more y x))))))

(defmacro ^::ana/numeric js-mod [num div]
  (core/list 'host* "(math.fmod(~{}, ~{}))" num div))

(defmacro ^::ana/numeric bit-not [x]
  (core/list 'host* "(bit.bnot(~{}))" x))

(defmacro ^::ana/numeric bit-and
  ([x y] (core/list 'host* "(bit.band(~{}, ~{}))" x y))
  ([x y & more] `(bit-and (bit-and ~x ~y) ~@more)))

;; internal do not use
(defmacro ^::ana/numeric unsafe-bit-and
  ([x y] (bool-expr (core/list 'host* "(bit.band(~{}, ~{}))" x y)))
  ([x y & more] `(unsafe-bit-and (unsafe-bit-and ~x ~y) ~@more)))

(defmacro ^::ana/numeric bit-or
  ([x y] (core/list 'host* "(bit.bor(~{}, ~{}))" x y))
...
(defmacro ^::ana/numeric bit-xor
  ([x y] (core/list 'host* "(bit.bxor(~{}, ~{}))" x y))
  ([x y & more] `(bit-xor (bit-xor ~x ~y) ~@more)))

(defmacro ^::ana/numeric bit-and-not
  ([x y] (core/list 'host* "(bit.band(~{}, bit.bnot(~{})))" x y))
  ([x y & more] `(bit-and-not (bit-and-not ~x ~y) ~@more)))

(defmacro ^::ana/numeric bit-clear [x n]
  (core/list 'host* "(bit.band(~{}, bit.bnot(bit.lshift(1, ~{}))))" x n))

(defmacro ^::ana/numeric bit-flip [x n]
  (core/list 'host* "(bit.bxor(~{}, bit.lshift(1, ~{})))" x n))

(defmacro ^::ana/numeric bit-test [x n]
  (core/list 'host* "(bit.band(~{}, bit.lshift(1, ~{})) ~= 0)" x n))

(defmacro ^::ana/numeric bit-shift-left [x n]
  (core/list 'host* "(bit.lshift(~{}, ~{}))" x n))

(defmacro ^::ana/numeric bit-shift-right [x n]
  (core/list 'host* "(bit.arshift(~{}, ~{}))" x n))

(defmacro ^::ana/numeric bit-shift-right-zero-fill [x n]
  (core/list 'host* "(bit.rshift(~{}, ~{}))" x n))

(defmacro ^::ana/numeric unsigned-bit-shift-right [x n]
  (core/list 'host* "(bit.rshift(~{}, ~{}))" x n))

(defmacro ^::ana/numeric bit-set [x n]
  (core/list 'host* "(bit.bor(~{}, bit.lshift(1, ~{})))" x n))

;; internal
(defmacro mask [hash shift]
  (core/list 'host* "(bit.band(bit.rshift(~{}, ~{}), 0x01f))" hash shift))

;; internal
(defmacro bitpos [hash shift]
  (core/list 'host* "(bit.lshift(1, ~{}))" `(mask ~hash ~shift)))
...
;; TODO: this needs to be expanded for Terra types
...
(def #^:private base-type
     {nil "nil"
      ...
      'string "string"
      'number "number"
      'array "table"  ;; TODO: this might be a mistake
      'function "function"
      'boolean "boolean"
      'default "_"})

;;TODO is this needed?
(def #^:private host-base-type
     {'js/Boolean "boolean"
      'js/String "string"
      'js/Array "table"
      'js/Number "number"
...
(defn warn-and-update-protocol [p type env]
  (when-not (= 'Object p)
    (if-let [var (cljs.terra.analyzer/resolve-existing-var (dissoc env :locals) p)]
      (do
        (when-not (:protocol-symbol var)
          (cljs.terra.analyzer/warning :invalid-protocol-symbol env {:protocol p}))
        (when (core/and (:protocol-deprecated cljs.terra.analyzer/*cljs-warnings*)
                (-> var :deprecated)
                (not (-> p meta :deprecation-nowarn)))
          (cljs.terra.analyzer/warning :protocol-deprecated env {:protocol p}))
        (when (:protocol-symbol var)
          (swap! env/*compiler* update-in [:cljs.terra.analyzer/namespaces]
            (fn [ns]
              (update-in ns [(:ns var) :defs (symbol (name p)) :impls]
                conj type)))))
      (when (:undeclared cljs.terra.analyzer/*cljs-warnings*)
        (cljs.terra.analyzer/warning :undeclared-protocol-symbol env {:protocol p})))))

(defn resolve-var [env sym]
  (let [ret (-> (dissoc env :locals)
              (cljs.terra.analyzer/resolve-var sym)
              :name)]
  ...
  (map
    (fn [meth]
      (let [arity (count (first meth))]
        `(set! ~(extend-prefix type-sym (symbol (core/str "cljs__core__IFn___invoke__arity__" arity)))
...
(defn add-proto-methods* [pprefix type type-sym [f & meths :as form]]
  (let [pf (core/str pprefix f)]
    (if (vector? (first meths))
      ;; single method case
      (let [meth meths]
        [`(set! ~(extend-prefix type-sym (core/str pf "__arity__" (count (first meth))))
            ~(with-meta `(fn ~@(adapt-proto-params type meth)) (meta form)))])
      (map (fn [[sig & body :as meth]]
             `(set! ~(extend-prefix type-sym (core/str pf "__arity__" (count sig)))
                ~(with-meta `(fn ~(adapt-proto-params type meth)) (meta form))))
        meths))))
...
(defmacro extend-type [type-sym & impls]
  (let [env &env
    ...
    (when (core/and (:extending-base-host-type cljs.terra.analyzer/*cljs-warnings*)
                    (host-base-type type-sym))
      (cljs.terra.analyzer/warning :extending-base-host-type env
          {:current-symbol type-sym :suggested-symbol (host-base-type type-sym)}))
...
(defn dt->et
  ...
  ([type specs fields inline]
    (let [annots {:cljs.terra.analyzer/type type
                  :cljs.terra.analyzer/fields fields
                  :protocol-impl true
...
(defn collect-protocols [impls env]
  (->> impls
      (filter core/symbol?)
      (map #(:name (cljs.terra.analyzer/resolve-var (dissoc env :locals) %)))
      (into #{})))
...
(defmacro deftype [t fields & impls]
  (let [r (:name (cljs.terra.analyzer/resolve-var (dissoc &env :locals) t))
    ...
    (if (seq impls)
      `(do
         ...
         (set! (.-cljs__lang__ctorStr ~t) ~(core/str r))
         (set! (.-cljs__lang__ctorPrWriter ~t) (fn [this# writer# opt#] (-write writer# ~(core/str r))))
      ...
      `(do
         ...
         (set! (.-cljs__lang__type ~t) true)
         (set! (.-cljs__lang__ctorStr ~t) ~(core/str r))
         (set! (.-cljs__lang__ctorPrWriter ~t) (fn [this# writer# opts#] (-write writer# ~(core/str r))))
         ~(build-positional-factory t r fields)
         ~t))))
...
(defmacro defrecord [rsym fields & impls]
  (let [rsym (vary-meta rsym assoc :internal-ctor true)
        r    (vary-meta
               (:name (cljs.terra.analyzer/resolve-var (dissoc &env :locals) rsym))
               assoc :internal-ctor true)]
    `(let []
       ~(emit-defrecord &env rsym r fields impls)
       (set! (.-cljs__lang__type ~r) true)
       (set! (.-cljs__lang__ctorPrSeq ~r) (fn [this#] (core/list ~(core/str r))))
       (set! (.-cljs__lang__ctorPrWriter ~r) (fn [this# writer#] (-write writer# ~(core/str r))))
       ~(build-positional-factory rsym r fields)
       ~(build-map-factory rsym r fields)
       ~r)))

(defmacro defprotocol [psym & doc+methods]
  (let [p (:name (cljs.terra.analyzer/resolve-var (dissoc &env :locals) psym))
        ...
        method (fn [[fname & sigs]]
                 (let [sigs (take-while vector? sigs)
                   ...
                   `(do
                      (set! (.. lua/basic_types_prot_functions ~(to-property fname)) (~'js* "{}"))
...
;;TODO: Com back here and see if CLJS-Lua is appropriate
(defmacro implements?
  "EXPERIMENTAL"
  [psym x]
  (let [p          (:name
                    (cljs.terra.analyzer/resolve-var
                      (dissoc &env :locals) psym))
        prefix     (protocol-prefix p)
        xsym       (bool-expr (gensym))
        [part bit] (fast-path-protocols p)
        msym       (symbol
                      (core/str "-cljs__lang__protocol_mask__partition" part "__"))]
    `(let [~xsym ~x]
       (if ~xsym
...
;;TODO Come back here and see if CLJS-Lua is appropriate
(defmacro satisfies?
...
(defmacro binding
  ...
  [bindings & body]
  (let [names (take-nth 2 bindings)]
    (cljs.terra.analyzer/confirm-bindings &env names)
    `(with-redefs ~bindings ~@body)))

;; TODO - this should be a done with `setmetatable` and a global Error table

(defmacro error-obj
  "Provides a host/platform independent way to generate errors"
  [msg & data]
  (apply lua-table (conj data msg :message)))
    ...
    (core/cond
      ...
      (let [keys (map first (partition 2 kvs))]
        (core/and (every? #(= (:op %) :constant)
                    (map #(cljs.terra.analyzer/analyze &env %) keys))
...
(defmacro hash-set
  ...
  ([& xs]
    (if (core/and (core/<= (count xs) 8)
                  (every? #(= (:op %) :constant)
                    (map #(cljs.terra.analyzer/analyze &env %) xs))
...
(defn js-obj* [kvs]
  (let [kvs-str (->> (repeat "[~{}]=~{}")
                     ...
                     (apply core/str))]
    (vary-meta
      (list* 'host* (core/str "({" kvs-str "})") (apply concat kvs))
      assoc :tag 'object))) ;; TODO: Should the tag be 'table ?
...
(def lua-table js-obj)

(defmacro alength [a]
  (vary-meta
    (core/list 'host* "(~{}).length" a)
    assoc :tag 'number))
...
(defn gen-apply-to-helper
  ...
  ([n]
     (let [prop (symbol (core/str "-cljs__core__IFn___invoke__arity__" n))
           f (symbol (core/str "cljs__core__IFn___invoke__arity__" n))]
       (if (core/<= n 20)
