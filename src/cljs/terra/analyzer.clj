...
(defmethod error-message :extending-base-host-type
  [warning-type info]
...
(defmethod error-message :invalid-arithmetic
  [warning-type info]
  (str (:host-op info) ", all arguments must be numbers, got " (:types info) " instead."))
...
;; TODO: Update for Terra specific globals
;; The following Lua globals are tables -  package, table, string, math, io, os, debug
(def host-globals '(module require package table string math io os debug))

;; JS globals are: '(alert window document console escape unescape
...
(def specials '#{if def fn* do let* loop* letfn* throw try recur new set! ns deftype* defrecord* . host* & quote})
...
(defmethod parse 'fn*
  ...
  (let [[name meths] (if (symbol? (first args))
    ...
    {:env env :op :fn :form form :name name-var :methods methods :variadic variadic
     ...
     :fndoc [(when variadic "@param {...*} var_args")]
     :max-fixed-arity max-fixed-arity
     :protocol-impl protocol-impl
     :protocol-inline protocol-inline
...
(defmethod parse 'host*
  [op env [_ jsform & args :as form] _]
  (when-not (string? jsform)
    (throw (error env "Invalid host* form")))
  (if args
    (disallowing-recur
         ...
         (let [types (map #(infer-tag env %) argexprs)]
           (when-not (every?
             ...
             (warning :invalid-arithmetic env
               {:host-op (-> form meta :host-op)
                :types (into [] types)}))))
       {:env env :op :host :segs (seg jsform) :args argexprs
    ...
    (let [interp (fn interp [^String s]
      ...
      {:env env :op :host :form form :code (apply str (interp jsform))
       :tag (or (-> form meta :tag)
                (and (-> form meta :numeric) 'number)
                nil)
       :host-op (-> form meta :host-op)
                ...
                (let [sym' (first form')
                  ...
                  (if (= sym' 'host*)
                    (vary-meta form' merge
                      (cond-> {:host-op (if (namespace sym) sym (symbol "cljs.core" (str sym)))}
...
(defn analyze-host-value
  ...
  (let [val (.val form)
    ...
    {:op :host-value
     :host-type (if (map? val) :object :array)
     :env env
     :form form
     :items items
     :children items
          ...
          (binding [reader/*alias-map* (or reader/*alias-map* {})]
            (let [form (if (instance? clojure.lang.LazySeq form)
              ...
              (cond
                ...
                (instance? JSValue form) (analyze-host-value env form)
