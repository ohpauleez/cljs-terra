...
(ns cljs.terra.compiler
  ...
  (:require [clojure.java.io :as io]
            ...
            [cljs.terra.analyzer :as ana]
            ;; There is source-mapping currently for Lua
            ;; This is left in, but will always be ignored during compilation
...
(def ^:dynamic *cljsterra-version*)

(defn cljsterra-version
    "Returns clojurescript terra version as a printable string."
  []
  (str
    (:major *cljsterra-version*)
    "."
    (:minor *cljsterra-version*)
    (when-let [i (:incremental *cljsterra-version*)]
      (str "." i))
    (when-let [q (:qualifier *cljsterra-version*)]
      (str "-" q))
    (when (:interim *cljsterra-version*)
      "-SNAPSHOT")) )
...
(def terra-reserved
  #{"and" "break" "do" "else" "elseif" "end" "for"
    ...
    "until" "while" "bit" "var" "struct" "union" "goto"
...
;; All strings in lua can be used to match
...
(defmethod emit* :host-value
  [{:keys [items host-type env]}]
  (emit-wrap env
    (if (= host-type :object)
      (do
        (emits "{")
...
(defn rename-to-tra
  "Change the file extension from .cljs to .t. Takes a File or a
  String. Always returns a String."
  [file-str]
  (clojure.string/replace file-str #"\.cljs$" ".t"))
